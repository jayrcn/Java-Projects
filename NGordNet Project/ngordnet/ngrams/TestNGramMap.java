...
public class TestNGramMap {
    ...
    public void testOnSmallFile() {
        ...
        // Request instances between 2007 & 2008
        TimeSeries reqCount = ngm.countHistory("request", 2007, 2008);
        assertEquals(697645.0, reqCount.get(2007), 1E-10);
        assertEquals(795265.0, reqCount.get(2008), 1E-10);

        TimeSeries totalCounts = ngm.totalCountHistory();
        assertEquals(28307904288.0, totalCounts.get(2007), 1E-10);

        // returns the relative weight of the word fish in each year between 1850 and 1933.
        TimeSeries reqWeight = ngm.weightHistory("request", 2007, 2008);
        assertEquals(697645.0/28307904288.0, reqWeight.get(2007), 1E-7);
        System.out.println(reqWeight.toString());
        TimeSeries apCount = ngm.countHistory("airport", 2007, 2008);
        assertEquals(175702.0, apCount.get(2007), 1E-10);

        List<String> fishAndDog = new ArrayList<>();
        fishAndDog.add("request");
        fishAndDog.add("airport");
        TimeSeries reqPlusApWeight1 = ngm.summedWeightHistory(fishAndDog);
        System.out.println(reqPlusApWeight1.toString());
        TimeSeries reqPlusApWeight = ngm.summedWeightHistory(fishAndDog, 2007, 2008);
        System.out.println(reqPlusApWeight.toString());
        double expectedReqPlusApWeight = (697645.0 + 175702.0) / 28307904288.0;
        assertEquals(expectedReqPlusApWeight, reqPlusApWeight.get(2007), 1E-10);
    }
